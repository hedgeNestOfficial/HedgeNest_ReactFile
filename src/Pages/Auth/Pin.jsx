...
const Pin = () => {
  ...
  // Quick form filler status evaluation
  ...
  useEffect(() => {
    ...
    // Intercept users who already finished their PIN config setup cycles
    if (
      tempUser?.createdAlready === true ||
      location.state?.createdAlready === true
    ) {
      toast.error("Transaction PIN already configured.");
  ...
  }, [user, tempUser, onboardingToken, userEmail, navigate, location.state]);
  ...
  const handleChange = (value, index, type) => {
    ...
    // Step Forward Focus Configuration
    if (value && index < 5) {
      const nextRef =
        type === "pin"
          ? inputRefs.current[`pin-${index + 1}`]
  ...
  const handleKeyDown = (e, index, type) => {
    ...
    // 🎯 FIXED: Re-added the programmatic backwards step .focus() trigger execution here
    if (e.key === "Backspace" && !currentArray[index] && index > 0) {
      const prevRef =
        type === "pin"
          ? inputRefs.current[`pin-${index - 1}`]
      ...
      <div className="form-container">
        <div className="signup-form-wrapper">
          ...
          <form className="auth-form" onSubmit={handleSubmitPin}>
            ...
            {/* CONFIRM PIN */}
            <div className="otp-inputs-row" style={{ marginTop: "16px" }}>
              <label
                style={{
                  fontSize: "14px",
                  fontWeight: "600",
            ...
            {/* 🎯 Updated button properties with native disable states */}
            <Button
              text={
                isLoading ? (
                  <div className="loader-wrapper">
              ...
              color={isFormComplete && !isLoading ? "#c9922a" : "#cbd5e1"}
            />
          </form>
        </div>
      </div>
