...
const LinkAccountModal = ({ isOpen, onClose, onSuccessRefresh }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ============================================================
      // CHECK IF USER HAS EXISTING ACCOUNT - SHOW ERROR IF YES
      // SILENTLY CONTINUE IF NO ACCOUNT (404 or empty array)
      // ============================================================
      try {
        const accountsResponse = await getLinkedAccounts(activeToken);
        const accountsArray = accountsResponse?.linkedAccounts || [];

        // If user has existing account(s), show error and stop
        if (accountsArray.length > 0) {
          setLoading(false);
          return toast.error("You already have a linked withdrawal account.");
        }
        // If accountsArray.length === 0, silently continue to linking
      } catch (checkError) {
        const status = checkError?.response?.status;

        // 404 means no accounts found - silently continue
        if (status === 404) {
          console.log("No existing accounts found. Proceeding to link...");
        } else {
          // For other errors, log but continue - assume no accounts
          console.warn("Error checking accounts:", checkError);
        }
      }

      // ============================================================
      // NO ACCOUNT FOUND OR ERROR - PROCEED WITH LINKING
      // ============================================================
