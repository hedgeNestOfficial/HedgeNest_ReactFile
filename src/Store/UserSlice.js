...
/**
 * ============================================================================
 * USER SLICE - REDUX STATE MANAGEMENT (UNIFIED TOKEN ARCHITECTURE)
 * ============================================================================
 */
...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    /**
     * 🔄 UPDATE TEMP USER TOKEN (UNIFIED PATH)
     * Captures intermediate token issued after OTP verification
     * and assigns it directly to the root token path.
     */
    updateTempUserToken: (state, action) => {
    ...
    /**
     * Finalizes authentication session post-login or PIN confirmation
     */
    login: (state, action) => {
      state.user = action.payload.user;
      state.wallet = action.payload.wallet || null;
    ...
    /**
     * Clears temporary signup values
     */
    clearTempUser: (state) => {
      state.tempUser = {
        email: "",
    ...
    /**
     * Merges update payloads into existing user state
     */
    updateUser: (state, action) => {
      const incomingData = action.payload?.data
        ? action.payload.data
